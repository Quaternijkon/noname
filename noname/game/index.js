...
class Game {
  ...
  updateConnectedCards() {
    game.broadcast((cards2) => {
      _status.sxrmConnectCards = cards2;
   ...
   */
  addConnectedCards(cards2) {
    if (get.itemtype(cards2) === "card") {
      cards2 = [cards2];
    }
    const shown = cards2.filter((card) => get.is.connectedCard(card)), hidden = cards2.filter((card) => !shown.includes(card));
    if (shown.length) {
   ...
   * @param { Card[] | Card } cards
   */
  removeConnectedCards(cards2) {
    if (get.itemtype(cards2) === "card") {
      cards2 = [cards2];
    }
    _status.sxrmConnectCards.removeArray(cards2);
    game.broadcastAll((cards3) => {
      cards3.forEach((card) => {
  ...
  async loadExtension(object) {
    ...
    extensionMenu.edit = {
      ...
      clear: true,
      onclick() {
        if (lib.extensionPack?.[name2] && object.editable !== false && lib.config.show_extensionmaker) {
...
${error}`);
  ...
  deleteFakeCards(cards2) {
    ...
    const fake = cards2.filter((card) => card.isFake && card._cardid), other = cards2.removeArray(fake), wild = [], map = {};
    fake.forEach((card) => {
